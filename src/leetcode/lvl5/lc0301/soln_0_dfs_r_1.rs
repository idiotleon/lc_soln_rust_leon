...
impl Solution {
    ...
    pub fn remove_invalid_parentheses(s: String) -> Vec<String> {
        ...
        Self::dfs(0, 0, s, Self::PAREN_OPEN, Self::PAREN_CLOSED, &mut ans);
        return ans;
    }
    fn dfs(
        lo_start: usize,
        ...
        for hi in hi_start..len_cs {
            ...
            for lo in lo_start..=hi {
                if chs[lo] == paren_closed && (lo == 0 || chs[lo - 1] != paren_closed) {
                    let former = chs[..lo].iter().collect::<String>();
                    let latter = chs[lo + 1..].iter().collect::<String>();
                    let deleted: String = format!("{}{}", former, latter);
                    Self::dfs(lo, hi, deleted, paren_open, paren_closed, res);
                }
            }
            return;
        }
        let reversed = chs.to_vec().into_iter().rev().collect::<String>();
        if paren_open == Self::PAREN_OPEN {
            Self::dfs(0, 0, reversed, Self::PAREN_CLOSED, Self::PAREN_OPEN, res);
        } else {
            res.push(reversed);
        }
    }
