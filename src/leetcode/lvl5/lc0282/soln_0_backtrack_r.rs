...
impl Solution {
    ...
    pub fn add_operators(num: String, target: i32) -> Vec<String> {
        ...
        Self::backtrack(&mut res, 0, 0, 0, &chs, target as i64, &mut ans);
        return ans;
    }
    fn backtrack(
        cur_res: &mut String,
        ...
        for idx in idx_start..len_cs {
            ...
            let len_s = cur_res.len();
            if idx_start == 0 {
                cur_res.push_str(&(cur.to_string()));
                Self::backtrack(cur_res, cur, cur, idx + 1, chs, target, res);
                cur_res.truncate(len_s);
            } else {
                cur_res.push(Self::SIGN_PLUS);
                cur_res.push_str(&(cur.to_string()));
                Self::backtrack(cur_res, prev + cur, cur, idx + 1, chs, target, res);
                cur_res.truncate(len_s);

                cur_res.push(Self::SIGN_MINUS);
                cur_res.push_str(&(cur.to_string()));
                Self::backtrack(cur_res, prev - cur, -cur, idx + 1, chs, target, res);
                cur_res.truncate(len_s);

                cur_res.push(Self::SIGN_MULTI);
                cur_res.push_str(&(cur.to_string()));
                Self::backtrack(
                    cur_res,
