...
impl Solution {
    pub fn garbage_collection(garbages: Vec<String>, travel: Vec<i32>) -> i32 {
        ...
        let idx_l_g: usize = Self::get_last_idx(&garbages, G);
        let idx_l_m: usize = Self::get_last_idx(&garbages, M);
        let idx_l_p: usize = Self::get_last_idx(&garbages, P);
        let mut ans: i32 = 0;
        ans += Self::get_time(&garbages, &travel, idx_l_g, G);
        ans += Self::get_time(&garbages, &travel, idx_l_m, M);
        ans += Self::get_time(&garbages, &travel, idx_l_p, P);
        return ans;
    }
